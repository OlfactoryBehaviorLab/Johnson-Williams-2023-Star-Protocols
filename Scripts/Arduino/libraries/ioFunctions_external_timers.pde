...
#include <C:\git\Olfalab_Voyeur_protocols_and_files\Arduino\libraries\lib\ADfunctions\ADfunctions.pde>
...
// event parameters
//volatile int *sniff, *treadmill;
//volatile unsigned long *lick, *lick2, *trig;  //*lick is lick buffer (a ring buffer)
#ifndef BUFFERS
#define BUFFERS 400
#endif
#ifndef TMBUFF
...
volatile int sniff[BUFFERS];
volatile int treadmill[TMBUFF];
volatile unsigned long lick[LICKBUFF];
volatile unsigned long lick2[LICKBUFF];
volatile unsigned long trig[TRIGBUFF];
...
boolean hasLicked(uint8_t pin, int16_t lickindex) {
  if(pin == 1) {  
...
void valveOnTimer(uint8_t valve, unsigned long time) {
  ...
  if (time) {  // if time == 0, don't do anything
    uint8_t i;
